...
describe "Bitcoin::Wallet::SimpleKeyStore" do
  ...
  it "should not allow the same label twice" do
    expect { @ks.new_key("test1") }.to raise_error(ArgumentError)
    expect { @ks.add_key({label: "test1", addr: "12345"}) }.to raise_error(ArgumentError)
    expect { @ks.import("foobar", "test1") }.to raise_error(ArgumentError)
  end

  it "should not allow invalid addrs" do
    expect { @ks.add_key(addr: "foobar") }.to raise_error(ArgumentError)
  end
